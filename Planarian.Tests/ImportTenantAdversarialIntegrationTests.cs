...
public sealed class ImportTenantAdversarialIntegrationTests(PostgresIntegrationFixture fixture) : IClassFixture<PostgresIntegrationFixture>
{
    ...
    public async Task CaveSyncCannotUpdateForeignCollidingCave()
    {
        ...
        await MakeBVisibleKeyCollide(d,b,"A01"); var before=await NormalizedDatabaseState.CaptureTenantAsync(d,b.AccountId); await using var db=d.CreateDbContext("a",a.AccountId); var p=new CaveImportPlanner(db,db.RequestUser);
    ...
    public async Task CaveSyncCannotDeleteForeignCave()
    {
        ...
        var before=await NormalizedDatabaseState.CaptureTenantAsync(d,b.AccountId); await using var db=d.CreateDbContext("a",a.AccountId); var p=new CaveImportPlanner(db,db.RequestUser);
    ...
    public async Task EntranceCannotAssociateToForeignCave()
    {
        ...
        var before=await NormalizedDatabaseState.CaptureTenantAsync(d,b.AccountId); await using var db=d.CreateDbContext("a",a.AccountId); var p=new EntranceImportPlanner(db,db.RequestUser);
    ...
    public async Task ForeignPrimaryEntranceDoesNotAffectAccountAPrimaryCount()
    {
        ...
        var before=await NormalizedDatabaseState.CaptureTenantAsync(d,b.AccountId); await using var db=d.CreateDbContext("a",a.AccountId); var p=new EntranceImportPlanner(db,db.RequestUser); await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.EntranceHeader+"\nA Primary,A01,1,true,35,-86,500,Survey Grade,0,Open,,,,,,\n");
