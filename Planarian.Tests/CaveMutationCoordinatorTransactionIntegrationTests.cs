...
public sealed class CaveMutationCoordinatorTransactionIntegrationTests(PostgresIntegrationFixture fixture)
    : IClassFixture<PostgresIntegrationFixture>
{
    ...
    public async Task CallerOwnedTransactionPublishesManagerUpdateAndAdvancesPointer()
    {
        ...
        await using (var db = database.CreateDbContext("manager", tenant.AccountId))
        {
            ...
            var coordinator = new CaveMutationCoordinator(db, db.RequestUser,
                new CavePublishedSnapshotReader(db, db.RequestUser));
