...
public sealed class TenantAdversarialIntegrationTests(PostgresIntegrationFixture fixture) : IClassFixture<PostgresIntegrationFixture>
{
    [Fact]
    public async Task AccountACannotSnapshotAccountBCave()
    {
        ...
        var a = await IntegrationTestData.SeedTenantAsync(database,'a'); var b = await IntegrationTestData.SeedTenantAsync(database,'b');
        await using var db = database.CreateDbContext("a",a.AccountId);
        await Assert.ThrowsAsync<InvalidOperationException>(() => new CavePublishedSnapshotReader(db,db.RequestUser).BuildAsync(b.CaveId));
    ...
    public async Task AccountACannotMutateAccountBCave()
    {
        ...
        var a = await IntegrationTestData.SeedTenantAsync(database,'a'); var b = await IntegrationTestData.SeedTenantAsync(database,'b');
        await using var db = database.CreateDbContext("a",a.AccountId);
        var reader = new CavePublishedSnapshotReader(db,db.RequestUser); var coordinator = new CaveMutationCoordinator(db,db.RequestUser,reader);
    ...
    private static async Task AssertB(PostgresTestDatabase database,TenantSeed b,string name)
    {
        await using var verify=database.CreateDbContext("b",b.AccountId);
