...
public sealed class ImportPreviewCommitEquivalenceTests(PostgresIntegrationFixture fixture):IClassFixture<PostgresIntegrationFixture>
{
    ...
    {
        await using var d=await fixture.CreateDatabaseAsync(nameof(CaveNoChangePreviewAndCommitAreEquivalent));var t=await IntegrationTestData.SeedTenantAsync(d,'a');await using var db=d.CreateDbContext("a",t.AccountId);var p=new CaveImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.CaveHeader+"\nCave A,County A,A01,1,AA,,,,,,,,,,,,,,,false,,\n");var plan=await p.PlanAsync(csv,true);var row=Assert.Single(plan.Caves);Assert.Equal(CaveImportAction.NoChange,row.Action);Assert.Equal("no change",Assert.Single(plan.CreatePreview(false)).Action);Assert.Empty(plan.CreatePreview(true));var before=await new CavePublishedSnapshotReader(db,db.RequestUser).BuildAsync(t.CaveId);var revision=(await db.Caves.IgnoreQueryFilters().SingleAsync(x=>x.Id==t.CaveId)).CurrentRevisionId;await ExecuteCaves(db,plan,"no-change.csv");db.ChangeTracker.Clear();var after=await new CavePublishedSnapshotReader(db,db.RequestUser).BuildAsync(t.CaveId);Assert.Equal(CaveSnapshotJson.Serialize(before),CaveSnapshotJson.Serialize(after));Assert.Equal(revision,(await db.Caves.IgnoreQueryFilters().SingleAsync(x=>x.Id==t.CaveId)).CurrentRevisionId);
    }

    [Fact]
    public async Task CaveSyncDeletionPreviewMatchesCommit()
    {
        await using var d=await fixture.CreateDatabaseAsync(nameof(CaveSyncDeletionPreviewMatchesCommit));var t=await IntegrationTestData.SeedTenantAsync(d,'a');await using var db=d.CreateDbContext("a",t.AccountId);var p=new CaveImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.CaveHeader+"\nReplacement,Replacement County,REP,2,AA,,,,10,2,1,1,,,,,,,,false,,Replacement\n");var plan=await p.PlanAsync(csv,true);var deletion=Assert.Single(plan.Deletions);Assert.Equal(t.CaveId,deletion.CaveId);var preview=Assert.Single(plan.CreatePreview(true),x=>x.Action=="delete");Assert.Equal("Cave A",preview.CaveName);Assert.Equal("A01",preview.CountyCode);Assert.Equal(1,preview.CountyCaveNumber);var revisionsBefore=await db.CaveRevisions.CountAsync(x=>x.CaveId==t.CaveId);await ExecuteCaves(db,plan,"delete.csv");Assert.False(await db.Caves.IgnoreQueryFilters().AnyAsync(x=>x.Id==t.CaveId));var tombstone=await db.CaveRevisions.Where(x=>x.CaveId==t.CaveId).OrderByDescending(x=>x.CreatedOn).FirstAsync();Assert.Equal(revisionsBefore+1,await db.CaveRevisions.CountAsync(x=>x.CaveId==t.CaveId));Assert.Equal(CaveRevisionOperation.Delete,tombstone.Operation);
    }
    ...
    {
        await using var d=await fixture.CreateDatabaseAsync($"preview_cave_{scenario}");var t=await IntegrationTestData.SeedTenantAsync(d,'a');await using var db=d.CreateDbContext("a",t.AccountId);var p=new CaveImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.CaveHeader+'\n'+row+'\n');var plan=await p.PlanAsync(csv,sync);var planned=Assert.Single(plan.Caves);Assert.Equal(expectedAction,planned.Action);var preview=Assert.Single(plan.CreatePreview(true));await ExecuteCaves(db,plan,$"{scenario}.csv");db.ChangeTracker.Clear();var snapshot=await new CavePublishedSnapshotReader(db,db.RequestUser).BuildAsync(planned.Id);AssertCavePreview(preview,snapshot);var cave=await db.Caves.IgnoreQueryFilters().SingleAsync(x=>x.Id==planned.Id);Assert.NotNull(cave.CurrentRevisionId);Assert.True(await db.CaveRevisions.AnyAsync(x=>x.Id==cave.CurrentRevisionId&&x.Source==CaveRevisionSource.Import));Assert.True(await db.CaveImportBatches.AnyAsync(x=>x.SourceFileName==$"{scenario}.csv"));
    }

    private async Task VerifyEntranceAsync(string scenario,bool sync,bool seedExisting,bool importPrimary)
    {
        await using var d=await fixture.CreateDatabaseAsync($"preview_entrance_{scenario}");var t=await IntegrationTestData.SeedTenantAsync(d,'a');if(seedExisting)await SeedPrimary(d,t);await using var db=d.CreateDbContext("a",t.AccountId);var p=new EntranceImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.EntranceHeader+$"\nImported,A01,1,{importPrimary.ToString().ToLowerInvariant()},35.123,-86.456,612,Survey Grade,20,Open,Wet,Sink,2026-08-01,Surveyor,Entrance description\n");var plan=await p.PlanAsync(csv,sync);var planned=Assert.Single(plan.Entrances);var preview=Assert.Single(plan.CreatePreview());await ExecuteEntrances(db,plan,$"{scenario}.csv");db.ChangeTracker.Clear();var snapshot=await new CavePublishedSnapshotReader(db,db.RequestUser).BuildAsync(t.CaveId);var committed=Assert.Single(snapshot.Entrances,x=>x.Id==planned.Id);AssertEntrancePreview(preview,committed);Assert.Equal(preview.EntranceCountChange,snapshot.Entrances.Count-(seedExisting?1:0));if(sync)Assert.DoesNotContain(snapshot.Entrances,x=>x.Id=="existing00");var cave=await db.Caves.IgnoreQueryFilters().SingleAsync(x=>x.Id==t.CaveId);Assert.NotNull(cave.CurrentRevisionId);Assert.True(await db.CaveRevisions.AnyAsync(x=>x.Id==cave.CurrentRevisionId&&x.Source==CaveRevisionSource.Import));
    }
    ...
    private static async Task SeedPrimary(PostgresTestDatabase d,TenantSeed t){await using var db=d.CreateDbContext("a",t.AccountId);var q=new TagType("Survey Grade",TagTypeKeyConstant.LocationQuality){Id=IdGenerator.Generate(),AccountId=t.AccountId,IsDefault=false};db.TagTypes.Add(q);await db.SaveChangesAsync();db.Entrances.Add(new Entrance{Id="existing00",CaveId=t.CaveId,LocationQualityTagId=q.Id,Name="Existing",IsPrimary=true,Location=new Point(new CoordinateZ(-86,35,500)){SRID=4326}});await db.SaveChangesAsync();}
