...
public sealed class ImportScaleBenchmarkTests(PostgresIntegrationFixture fixture,ITestOutputHelper output):IClassFixture<PostgresIntegrationFixture>
{
    ...
    {
        var parse=Stopwatch.StartNew();var rows=ProbeCaves(text);parse.Stop();var sql=new SqlTimingInterceptor();var saves=new SaveMetricsInterceptor();await using var db=Context(d,account,sql,saves);var planner=new CaveImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(text);var p=Stopwatch.StartNew();var plan=await planner.PlanAsync(csv,sync);p.Stop();sql.Reset();saves.Reset();var r=new CavePublishedSnapshotReader(db,db.RequestUser);var e=Stopwatch.StartNew();await new CaveImportExecutor(db,db.RequestUser,r,new ImportRevisionPublisher(db,db.RequestUser)).ExecuteAsync(plan,phase+".csv");e.Stop();return new(phase,rows,parse.Elapsed.TotalMilliseconds,p.Elapsed.TotalMilliseconds,e.Elapsed.TotalMilliseconds,plan.Caves.Count(x=>x.Action==CaveImportAction.Insert),plan.Caves.Count(x=>x.Action==CaveImportAction.Update),plan.Caves.Count(x=>x.Action==CaveImportAction.NoChange),plan.Deletions.Count,sql.Items.Count,sql.Items.Count(x=>x.Sql.Contains("SELECT",StringComparison.OrdinalIgnoreCase)),sql.Items.Where(x=>x.Sql.Contains("INSERT",StringComparison.OrdinalIgnoreCase)||x.Sql.Contains("UPDATE",StringComparison.OrdinalIgnoreCase)||x.Sql.Contains("DELETE",StringComparison.OrdinalIgnoreCase)).Sum(x=>x.Duration.TotalMilliseconds),sql.Items.Where(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)).Sum(x=>x.Duration.TotalMilliseconds),saves.Saves,saves.TrackedHighWater);
    }
    private async Task<EntranceMetrics> RunEntrances(PostgresTestDatabase d,string account,string phase,string text,bool sync)
    {
        var parse=Stopwatch.StartNew();var rows=ProbeEntrances(text);parse.Stop();var sql=new SqlTimingInterceptor();var saves=new SaveMetricsInterceptor();await using var db=Context(d,account,sql,saves);var planner=new EntranceImportPlanner(db,db.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(text);var p=Stopwatch.StartNew();var plan=await planner.PlanAsync(csv,sync);p.Stop();sql.Reset();saves.Reset();var r=new CavePublishedSnapshotReader(db,db.RequestUser);var e=Stopwatch.StartNew();await new EntranceImportExecutor(db,db.RequestUser,r,new ImportRevisionPublisher(db,db.RequestUser)).ExecuteAsync(plan,phase+".csv");e.Stop();return new(phase,rows,plan.Targets.Count,parse.Elapsed.TotalMilliseconds,p.Elapsed.TotalMilliseconds,e.Elapsed.TotalMilliseconds,sql.Items.Count,sql.Items.Count(x=>x.Sql.Contains("SELECT",StringComparison.OrdinalIgnoreCase)),sql.Items.Where(x=>x.Sql.Contains("INSERT",StringComparison.OrdinalIgnoreCase)||x.Sql.Contains("UPDATE",StringComparison.OrdinalIgnoreCase)||x.Sql.Contains("DELETE",StringComparison.OrdinalIgnoreCase)).Sum(x=>x.Duration.TotalMilliseconds),sql.Items.Where(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)).Sum(x=>x.Duration.TotalMilliseconds),saves.Saves,saves.TrackedHighWater);
    }
    private static PlanarianDbContext Context(PostgresTestDatabase d,string account,SqlTimingInterceptor sql,SaveMetricsInterceptor saves){var options=new DbContextOptionsBuilder<PlanarianDbContext>().UseNpgsql(d.ConnectionString,o=>{o.MigrationsAssembly("Planarian.Migrations");o.UseNetTopologySuite();}).AddInterceptors(sql,saves).Options;var db=new PlanarianDbContext(options);db.RequestUser=new RequestUser(db){Id="bench",AccountId=account,FirstName="Scale",LastName="Benchmark"};return db;}
    ...
    public sealed record CaveMetrics(string Phase,int Rows,double ParseProbeMs,double PlanningMs,double ExecutionMs,int Inserts,int Updates,int NoChange,int Deletes,int Commands,int Selects,double WriteSqlMs,double RevisionSqlMs,int SaveChanges,int TrackedHighWater);
    public sealed record EntranceMetrics(string Phase,int Rows,int Targets,double ParseProbeMs,double PlanningMs,double ExecutionMs,int Commands,int Selects,double WriteSqlMs,double RevisionSqlMs,int SaveChanges,int TrackedHighWater);
}
