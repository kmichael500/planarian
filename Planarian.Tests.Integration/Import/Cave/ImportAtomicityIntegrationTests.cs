...
public sealed class ImportAtomicityIntegrationTests(PostgresTestServer fixture):IClassFixture<PostgresTestServer>
{
    ...
    {
        await using var d=await fixture.CreateDatabaseAsync(nameof(CaveSecondRecordLateFailureRollsBackCompleteState));var t=await TestDataScenarios.CreatePublishedCaveScenarioAsync(d,'a');CaveImportPlan plan;
        await using(var pctx=d.CreateDbContext("a",t.AccountId)){var p=new CaveImportPlanningWorkflow(pctx,pctx.RequestUser);await using var csv=ImportDryRunIntegrationTests.CsvStream(ImportDryRunIntegrationTests.CaveHeader+"\nRecord A,New County,NEW,50,AA,,,,100,20,5,1,Rollback Geology,,,,,,,false,,A\nRecord B,New County,NEW,51,AA,,,,110,21,6,2,Rollback Geology,,,,,,,false,,B\n");plan=await p.PlanAsync(csv,false);Assert.Equal(2,plan.Caves.Count);Assert.Contains(plan.TagCreations,x=>x.Name=="Rollback Geology");Assert.Single(plan.CountyCreations);}
        var before=await NormalizedDatabaseState.CaptureAllAsync(d);var fault=new FailAtRevisionPublicationInterceptor();
        await using(var db=d.CreateDbContext("a",t.AccountId,fault)){var r=new CavePublishedSnapshotRepository(db,db.RequestUser);var error=await Assert.ThrowsAsync<DbUpdateException>(()=>new CaveImportExecutionRepository(db,db.RequestUser,r,new CaveImportRevisionRepository(db,db.RequestUser)).ExecuteAsync(plan,"rollback-cave.csv"));Assert.IsType<InvalidOperationException>(error.InnerException);Assert.Contains("deterministic late failure",error.InnerException!.Message);}
    ...
    {
        await using var d=await fixture.CreateDatabaseAsync(nameof(EntranceSecondRecordLateFailureRollsBackCompleteState));var t=await TestDataScenarios.CreatePublishedCaveScenarioAsync(d,'a');
