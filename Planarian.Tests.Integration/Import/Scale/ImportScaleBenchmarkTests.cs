...
public sealed class ImportScaleBenchmarkTests(PostgresTestServer fixture,ITestOutputHelper output):IClassFixture<PostgresTestServer>
{
    ...
    {
        var sql=new SqlTimingInterceptor();var saves=new SaveMetricsInterceptor();await using var db=Context(d,account,sql,saves);
        await using var csv=ImportDryRunIntegrationTests.CsvStream(text);var parse=Stopwatch.StartNew();var records=await new CaveImportCsvParser().ParseAsync(csv);parse.Stop();
        var load=Stopwatch.StartNew();var state=await new CaveImportPlanningRepository(db,db.RequestUser).LoadAsync(records,sync);load.Stop();
        var planning=Stopwatch.StartNew();var plan=new CaveImportPlanner().Plan(records,state,sync);planning.Stop();
        sql.Reset();saves.Reset();var r=new CavePublishedSnapshotRepository(db,db.RequestUser);var e=Stopwatch.StartNew();await new CaveImportExecutionRepository(db,db.RequestUser,r,new CaveImportRevisionRepository(db,db.RequestUser)).ExecuteAsync(plan,phase+".csv");e.Stop();var writes=sql.Items.Where(x=>IsWrite(x.Sql)).ToList();return new(phase,records.Count,parse.Elapsed.TotalMilliseconds,load.Elapsed.TotalMilliseconds,planning.Elapsed.TotalMilliseconds,e.Elapsed.TotalMilliseconds,plan.Caves.Count(x=>x.Action==CaveImportAction.Insert),plan.Caves.Count(x=>x.Action==CaveImportAction.Update),plan.Caves.Count(x=>x.Action==CaveImportAction.NoChange),plan.Deletions.Count,sql.Items.Count,sql.Items.Count(x=>x.Sql.Contains("SELECT",StringComparison.OrdinalIgnoreCase)),writes.Count,writes.Count(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)),writes.Sum(x=>x.Duration.TotalMilliseconds),writes.Where(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)).Sum(x=>x.Duration.TotalMilliseconds),saves.Saves,saves.TrackedHighWater);
    }
    private async Task<EntranceMetrics> RunEntrances(PostgresTestDatabase d,string account,string phase,string text,bool sync)
    {
        var sql=new SqlTimingInterceptor();var saves=new SaveMetricsInterceptor();await using var db=Context(d,account,sql,saves);
        await using var csv=ImportDryRunIntegrationTests.CsvStream(text);var parse=Stopwatch.StartNew();var records=await new EntranceImportCsvParser().ParseAsync(csv);parse.Stop();
        var load=Stopwatch.StartNew();var state=await new EntranceImportPlanningRepository(db,db.RequestUser).LoadAsync(records);load.Stop();
        var planning=Stopwatch.StartNew();var plan=new EntranceImportPlanner().Plan(records,state,sync);planning.Stop();
        sql.Reset();saves.Reset();var r=new CavePublishedSnapshotRepository(db,db.RequestUser);var e=Stopwatch.StartNew();await new EntranceImportExecutionRepository(db,db.RequestUser,r,new CaveImportRevisionRepository(db,db.RequestUser)).ExecuteAsync(plan,phase+".csv");e.Stop();var writes=sql.Items.Where(x=>IsWrite(x.Sql)).ToList();return new(phase,records.Count,plan.Targets.Count,parse.Elapsed.TotalMilliseconds,load.Elapsed.TotalMilliseconds,planning.Elapsed.TotalMilliseconds,e.Elapsed.TotalMilliseconds,sql.Items.Count,sql.Items.Count(x=>x.Sql.Contains("SELECT",StringComparison.OrdinalIgnoreCase)),writes.Count,writes.Count(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)),writes.Sum(x=>x.Duration.TotalMilliseconds),writes.Where(x=>x.Sql.Contains("\"CaveRevisions\"",StringComparison.Ordinal)).Sum(x=>x.Duration.TotalMilliseconds),saves.Saves,saves.TrackedHighWater);
    }
    ...
    public sealed record CaveMetrics(string Phase,int Rows,double ParseMs,double StateLoadMs,double PurePlanningMs,double ExecutionMs,int Inserts,int Updates,int NoChange,int Deletes,int Commands,int Selects,int WriteCommands,int RevisionWriteCommands,double WriteSqlMs,double RevisionSqlMs,int SaveChanges,int TrackedHighWater);
    public sealed record EntranceMetrics(string Phase,int Rows,int Targets,double ParseMs,double StateLoadMs,double PurePlanningMs,double ExecutionMs,int Commands,int Selects,int WriteCommands,int RevisionWriteCommands,double WriteSqlMs,double RevisionSqlMs,int SaveChanges,int TrackedHighWater);
}
