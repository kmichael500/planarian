...
const ObjectiveTypeTagComponent = (props: ObjectiveTypeTagComponentProps) => {
  ...
  const handleClose = async (removedTag: SelectListItem<string>) => {
    try {
      await TripObjectiveService.DeleteTag(
        removedTag.value,
        props.tripObjectiveId
      );
  ...
  const handleSelectChange = async (value: string) => {
    ...
    if (value && tagAlreadyAdded === undefined) {
      ...
      try {
        await TripObjectiveService.AddTag(value, props.tripObjectiveId);
        var selectedTag = tagTypes.find((tag) => tag.value === value);
        if (!selectedTag) {
          return;
        }
...
export { ObjectiveTypeTagComponent };
