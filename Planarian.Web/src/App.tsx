...
import { getAppContentStyle } from "./Configuration/Layout/AppContentLayout";
...
export const AppShell: React.FC = () => {
  const { isInitialized, isLoading, initializedError, logout } =
    useContext(AppContext);
  const location = useLocation();
  const contentStyle = getAppContentStyle(location.pathname);
  ...
  return (
    ...
    >
      ...
      <Row>
        ...
        {initializedError?.errorCode === ApiExceptionType.Unauthorized && (
          <Col span={24}>
            <PlanarianButton
              icon={<RedoOutlined />}
              onClick={async () => {
